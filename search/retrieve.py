...
class Retrieval:
    def __init__(self, name_index_path, mechanic_index_path):
        name_inv_dt = self.load_index(name_index_path)
        mechanic_inv_dt = self.load_index(mechanic_index_path)
        self.inv_dt = {}
        self.inv_dt['name'] = name_inv_dt
        self.inv_dt['mechanic'] = mechanic_inv_dt
    ...
    def phrase_search(self, query, search_type='name'):
    ...
    def proximity_search(self, query, search_type='name'):
    ...
    def ranked_retrieval(self, query, id_list, type='name'):
        res = []
...
if __name__ == '__main__':
    ...
    # this_time = time.time()
    onlineIR = Retrieval('index_name.txt', 'index_mechanic.txt')

    while True:
        query = input('Please input the query: ')
        phrase_id= onlineIR.search(query, 'name')
        print(phrase_id)
